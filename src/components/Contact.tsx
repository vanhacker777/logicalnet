...
export default function Contact() {
  ...
  const inputClass = "w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand/50 focus:ring-1 focus:ring-brand/50 transition-all text-sm";
  const labelClass = "block text-gray-600 text-sm mb-2";

  return (
    <section id="contacto" className="py-32 relative" style={{background: 'linear-gradient(180deg, #ffffff 0%, #f5f7fa 50%, #ffffff 100%)'}}>
      <div className="max-w-3xl mx-auto px-6">
        <div ref={titleRef} className="text-center mb-16">
          ...
          <div className={`scroll-hidden scroll-hidden-delay-2 ${titleVisible ? 'scroll-visible' : ''} flex items-center justify-center gap-6 mt-6 text-sm`}>
            <a href="tel:+34601475239" className="text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" /></svg>
              601 475 239
            </a>
            <a href="https://wa.me/34601475239?text=Hola,%20me%20interesa%20vuestros%20servicios" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-2">
        ...
        {status === 'sent' ? (
          ...
          <form ref={formRef} onSubmit={handleSubmit} className={`scroll-hidden ${formVisible ? 'scroll-visible' : ''}`}>
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className={`flex items-center gap-2 ${step >= 1 ? 'text-brand' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border ${step >= 1 ? 'bg-brand/20 border-brand/40 text-brand' : 'border-gray-200 text-gray-400'}`}>1</div>
                <span className="text-sm font-medium hidden sm:inline">Datos</span>
              </div>
              <div className={`flex-1 h-px ${step >= 2 ? 'bg-brand' : 'bg-gray-200'} transition-colors duration-500`} />
              <div className={`flex items-center gap-2 ${step >= 2 ? 'text-brand' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border ${step >= 2 ? 'bg-brand/20 border-brand/40 text-brand' : 'border-gray-200 text-gray-400'}`}>2</div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select name="asunto" value={formData.asunto} onChange={handleChange} required className={inputClass}>
                    <option value="" className="bg-white">Selecciona</option>
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="px-6 py-4 rounded-full text-sm font-medium border border-gray-200 text-gray-500 hover:text-gray-900 hover:border-gray-300 transition-all"
                  >
                    Atrás
                  </button>
                  <button
            ...
            {status === 'error' && (
              <p className="text-red-500 text-sm text-center mt-4">Error al enviar. Inténtalo de nuevo o contáctanos por WhatsApp.</p>
            )}
          </form>
        )}
      </div>
